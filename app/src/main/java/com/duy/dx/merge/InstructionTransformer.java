...
final class InstructionTransformer {
    private final com.duy.dx.io.CodeReader reader;

    private DecodedInstruction[] mappedInstructions;
    private int mappedAt;
    private com.duy.dx.merge.IndexMap indexMap;

    public InstructionTransformer() {
        this.reader = new com.duy.dx.io.CodeReader();
    ...
    private class GenericVisitor implements com.duy.dx.io.CodeReader.Visitor {
        @Override
    ...
    private class StringVisitor implements com.duy.dx.io.CodeReader.Visitor {
        @Override
        public void visit(DecodedInstruction[] all, DecodedInstruction one) {
            int stringId = one.getIndex();
            int mappedId = indexMap.adjustString(stringId);
            boolean isJumbo = (one.getOpcode() == com.duy.dx.io.Opcodes.CONST_STRING_JUMBO);
            jumboCheck(isJumbo, mappedId);
            mappedInstructions[mappedAt++] = one.withIndex(mappedId);
        }
    }

    private class FieldVisitor implements com.duy.dx.io.CodeReader.Visitor {
        @Override
        public void visit(DecodedInstruction[] all, DecodedInstruction one) {
    ...
    private class TypeVisitor implements com.duy.dx.io.CodeReader.Visitor {
        @Override
        public void visit(DecodedInstruction[] all, DecodedInstruction one) {
    ...
    private class MethodVisitor implements com.duy.dx.io.CodeReader.Visitor {
        @Override
        public void visit(DecodedInstruction[] all, DecodedInstruction one) {
    ...
    private class MethodAndProtoVisitor implements com.duy.dx.io.CodeReader.Visitor {
        @Override
        public void visit(DecodedInstruction[] all, DecodedInstruction one) {
            int methodId = one.getIndex();
